...
- **昼夜循环**（480s 全周期）、**Minecraft 风格生物系统**（24m 兽群网格确定性生成 + 密度上限 16、24–128m 生成环带、>128m 卸载/<96m 重载、被杀生物永久消失、兽群与格子掩码随存档/跨星系档案持久化；淡入淡出 + 散步/休息状态机 + 行走摇摆/弹跳/呼吸/受击程序化动画）、**Kenney CC0 音效库（32 条，含引擎/喷气/爆炸/扫描/UI 全套 + 主音量控制）**；
