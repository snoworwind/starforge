...
- **太空飞船**：CC0 GLB 飞船模型（等级差异化 C/B/A/S）、登船/下船、燃料加注、大气层飞行（经纬环绕、地形碰撞、再入俯冲保护、冲出大气交棒）、太空飞行（姿态/滚转/脉冲引擎/氚消耗）、恒星高温危险、**小行星群（GLB 陨石，可击碎掉落氚/金）**；
- **太空战斗**：鼠标左键双发激光弹道、武器威力随等级（C1/B1/A2/S4）、**访客船队巡航 + 可击毁（战利品直入货仓 + 信用点，35-65s 补员）**；
...
- **星系与曲率跃迁**：初始星系 5 行星 + 空间站；随机星系生成（4~7 颗星球、市场波动、站体与星球分离校验）；星系地图（55 邻域 + 回家锁定）、脉冲冲刺自动跃迁、15s 曲速动画、抵达新星系（跨星系标记随档案换档）；
...
- **昼夜循环**（480s 全周期）、生物（CC0 GLB 动物模型 + 生态类型映射 + 死亡掉落）、**Kenney CC0 音效库（32 条，含引擎/喷气/爆炸/扫描/UI 全套 + 主音量控制）**。
...
- 星系 3D 星图与跃迁粒子视觉（以列表 + 拉伸镜头呈现）；
- 高空体积云、天气粒子、空间站访客泊站动画与站体护盾。
