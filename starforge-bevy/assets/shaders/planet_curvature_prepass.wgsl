// Depth/normal prepass counterpart of planet_curvature.wgsl.
#import bevy_pbr::{
    mesh_bindings::mesh,
    mesh_functions,
...
fn curve_position(flat: vec3<f32>) -> vec3<f32> {
    ...
    return mix(flat, sphere, smoothstep(curve.profile.y, curve.profile.z, distance));
}
...
fn vertex(vertex_in: Vertex) -> VertexOutput {
    ...
    let tangent_x = curve_position(flat + vec3<f32>(1.0, -flat_normal.x / ny, 0.0)) - curved;
    let tangent_z = curve_position(flat + vec3<f32>(0.0, -flat_normal.z / ny, 1.0)) - curved;
